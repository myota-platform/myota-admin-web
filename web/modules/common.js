...
function setView(name) { state.currentView = name; document.querySelectorAll('.view').forEach(v => v.hidden = v.id !== `${name}-view`); document.querySelectorAll('.nav-item').forEach(b => b.classList.toggle('active', b.dataset.view === name)); $('breadcrumb').textContent = name === 'master-data' ? 'Master data' : name === 'geodata-imports' ? 'Geodata imports' : name === 'entity-map' ? 'Entity map' : name === 'entity-management' ? 'Entity management' : name[0].toUpperCase() + name.slice(1); $('sidebar').classList.remove('open'); const fn = {dashboard:renderDashboard,programmes:renderProgrammes,'master-data':renderMasterData,geodata:renderGeoReview,'entity-management':renderEntityManagement,'entity-map':renderEntityMap,'geodata-imports':renderGeodataImports,policies:renderPolicies,awards:renderAwards,content:renderContent,identity:renderIdentity,activity:renderActivity}[name]; if (fn) fn(); }
