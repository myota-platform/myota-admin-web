...
async function loadImportCategories(programme) {
  if (!programme) return [];
  const data = await api(`/v1/programmes/${encodeURIComponent(programme)}/entity-types`);
  return data.items || data.entityTypes || [];
}
...
async function refreshImportRuns() {
  try {
    ...
    $('geo-import-runs').innerHTML = (data.items || []).map(run => `<div class="table-row"><span><strong>${esc(run.filename || run.format || 'Dataset import')}</strong><small>${esc(run.adapter || '')} · ${esc(run.entityType || '')} · ${esc(run.queuedAt || run.startedAt || '')}</small></span><span class="status-pill ${String(run.status || '').toLowerCase()}">${esc(run.status || 'QUEUED')}</span></div>`).join('') || '<p class="muted empty">No imports have been queued yet.</p>';
  } catch (error) { $('geo-import-runs').innerHTML = `<div class="error-card">${esc(error.message)}</div>`; }
}

async function updateImportProgramme() {
  const categories = await loadImportCategories($('geo-import-programme').value);
  $('geo-import-category').innerHTML = categories.map(category => `<option value="${esc(category.code)}">${esc(category.label || category.code)} · ${esc((category.geometryTypes || [category.geometry || '']).join(', '))}</option>`).join('');
  if (!categories.length) $('geo-import-category').innerHTML = '<option value="">No categories assigned to this programme</option>';
}

async function submitGeodataImport(event) {
  event.preventDefault();
  const format = $('geo-import-format').value;
  const programmeSlug = $('geo-import-programme').value;
  const entityType = $('geo-import-category').value;
  if (!programmeSlug || !entityType) { notify('Choose a programme and feature category before importing.', 'error'); return; }
