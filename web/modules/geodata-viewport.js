...
function bindGeoEntityControls() {
  ...
  const geometryType = entity.geometry?.type || 'Polygon';
  const type = geometryType === 'Point' ? 'POINT' : geometryType === 'LineString' ? 'LINESTRING' : geometryType === 'MultiLineString' ? 'MULTILINESTRING' : geometryType === 'MultiPolygon' ? 'MULTIPOLYGON' : 'POLYGON';
  const section = document.createElement('section');
  section.id = 'geo-entity-controls';
  section.className = 'geo-status-editor';
  section.innerHTML = `<h3>GIS administration</h3><p class="field-help">Global and GIS administrators can convert between the supported GeoJSON geometry types. The previous geometry remains in audit history.</p><div class="geo-status-grid"><label>Geometry type<select id="geo-geometry-type"><option value="POINT" ${type === 'POINT' ? 'selected' : ''}>Point</option><option value="LINESTRING" ${type === 'LINESTRING' ? 'selected' : ''}>LineString</option><option value="MULTILINESTRING" ${type === 'MULTILINESTRING' ? 'selected' : ''}>MultiLineString</option><option value="POLYGON" ${type === 'POLYGON' ? 'selected' : ''}>Polygon</option><option value="MULTIPOLYGON" ${type === 'MULTIPOLYGON' ? 'selected' : ''}>MultiPolygon</option></select></label><label>Change note<textarea id="geo-type-note" placeholder="Explain why the geometry type changed"></textarea></label><button class="secondary" id="geo-save-type" type="button" ${entity.status === 'RETIRED' ? 'disabled' : ''}>Save type</button></div>${entity.status === 'REJECTED' ? '<div class="form-actions"><button class="danger-button" id="geo-delete-rejected" type="button">Delete rejected entity permanently</button></div><p class="field-help">Deletion removes the entity, related conflation candidates, and its audit record. It cannot be undone.</p>' : ''}`;
