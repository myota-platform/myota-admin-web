...
function renderGeoGeometryEditor() {
  const panel = $('geo-geometry-editor');
  if (!panel) return;
  const entity = state.geoSelected;
  if (geoWorkspaceMode !== 'management' || !entity) {
    panel.hidden = true;
    panel.innerHTML = '';
    return;
  }
